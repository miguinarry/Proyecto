...
void ModBus::SetTarget(int Target){ this->Target = Target; }
...
void ModBus::SendRead(unsigned char *Buffer, unsigned int Address, unsigned int NumberOfRegisters){
	...
	*/
	this->Address = Address;
	this->Storage = 9 + 2 * NumberOfRegisters;
	// Transaction ID
	Buffer[0] = (TransactionID >> 8) & 0xFF;
	Buffer[1] = TransactionID & 0xFF;
	// Protocol ID => ModBus=0
...
int ModBus::ModBusReceived(unsigned char *ReceivedBuffer, unsigned char *SendBuffer){
	...
	*/
	...
	if (ReceivedTransactionID >= 0xFFFF)
		TransactionID = 0;
	else
	...
	if (Length != 0x06 && type==SERVER)
		return ERROR_LENGTH; //Show error message


	unsigned int Addres, Value;
	switch (type){
	...
	// Manage the answer made in the client
	case CLIENT:
		// Check Function Code
		if (ReceivedBuffer[7] == 0x04){ // Read request
			int Position = 0;
			if (Target == TARGET_MOTOR) Position = 3; 

			for (unsigned int i = 0; i < (Length - 3) / 2; i++){
				unsigned int Value;
				Value = (0xFF & (unsigned int)ReceivedBuffer[9+2*i]) << 8;
				Value += 0xFF & (unsigned int)ReceivedBuffer[10+2*i];
				Memory->Write(Address + Position++, Value);
			}
			return RESULT_OK;
			...
			// Get address
			Addres = (0xFF & (unsigned int)ReceivedBuffer[8]) << 8;
			Addres += 0xFF & (unsigned int)ReceivedBuffer[9];
